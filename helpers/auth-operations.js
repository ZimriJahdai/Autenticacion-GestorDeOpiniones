...
export const registerUserHelper = async (userData) => {
  try {
    ...
    if (profilePicture) {
      ...
      // Detectar si es un archivo local (viene de multer)
      const isLocalFile =
      ...
      console.log('   Path:', profilePicture);
      ...
      if (isLocalFile) {
        try {
          ...
          console.log('Uploading to Cloudinary as:', cloudinaryFileName);
          ...
          console.log('Upload successful:', profilePictureToStore);
        } catch (err) {
          console.error(
